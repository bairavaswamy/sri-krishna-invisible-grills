...
import { generateAntiBirdNetService } from "../../components/seo/antiBirdNetInstallationGenerator";
...
export async function generateMetadata({
  ...
  const page = generateAntiBirdNetService(
    location,
    locations,
...
const url = `https://rohiniinvisiblegrills.com/anti-bird-net-installation/${params.slug}`;
...
function getLocationImage(location: string) {
  ...
  const index = Math.floor(rand() * locationImagesForAntiBirdNetInstallation.length);

  return locationImagesForAntiBirdNetInstallation[index];
}
...
function getLocationImageInProduct(location: string) {
  ...
  const index = Math.floor(rand() * BalconySafetyAntiBirdNetInstallation.length);

  return BalconySafetyAntiBirdNetInstallation[index];
}
...
export default function Page({ params }: { params: { slug: string, sections: Section[]; } }) {
  ...
  const url = `https://rohiniinvisiblegrills.com/anti-bird-net-installation/${slugify(location)}`;
